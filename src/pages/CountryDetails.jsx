...
const CountryDetails = () => {
  ...
  // Remonter en haut à l'ouverture de la page (ou au changement de pays)
  useEffect(() => {
    window.scrollTo(0, 0);
  }, [code]);
  ...
  useEffect(() => {
    ...
    countriesAPI.getCountriesByCodes(country.borders).then((list) => {
      const map = {};
      list.forEach((c) => {
        if (c?.cca3 && c?.name?.common) map[c.cca3] = c.name.common;
      });
      setBorderNames(map);
    });
          ...
          <div className="card-body p-3 p-md-4">
            ...
            {funFactLoading ? (
              ...
              <>
                ...
                <a
                  href={`https://fr.wikipedia.org/wiki/${encodeURIComponent(country.name?.common || '')}`}
                  target="_blank"
